/* ══ <finna-chat-clip> — the Freya clip, as the page carries it ══════════════════════════════
   ...
   properties on :root) pass through. Autoplays, loops at 23s, takes no input, and plays in
   full on every device — it does not read Reduce Motion. A DEMONSTRATION with
...
(function () {
  ...
  /* REDUCED MOTION (22 Sep 2026, Esben) — the clip DOES NOT read the setting. It froze on
     a poster on any phone with Reduce Motion on, which read as broken. His call: the scene
     is the section's content, so it plays in full everywhere, the same as the invest clip's
     SMIL in home-06, which never honoured the setting either. */
  ...
  class FinnaChatClip extends HTMLElement {
    connectedCallback() {
      ...
      const apply = () => {
        ...
        playing = true; clear(); startAt = performance.now(); raf = requestAnimationFrame(frame);
      };
      apply();
    }
  }
